...
import net.markwalder.vtestmail.testutils.Pop3Client;
...
class Pop3ServerTest {
	...
	private void testEncryption(String sslProtocol, boolean useStartTLS) throws IOException, MessagingException, InterruptedException {
		...
		try (Pop3Server server = new Pop3Server(store)) {
			...
			// prepare: POP3 client
			net.markwalder.vtestmail.testutils.Pop3Client client;
			if (useStartTLS) {
				client = net.markwalder.vtestmail.testutils.Pop3Client.forServer(server)
	...
	private void testAuthentication(String authType, boolean encrypted, boolean wrongPassword) throws IOException, InterruptedException {
		...
		try (Pop3Server server = new Pop3Server(store)) {
			...
			// prepare: POP3 client
			net.markwalder.vtestmail.testutils.Pop3Client.Pop3ClientBuilder clientBuilder = net.markwalder.vtestmail.testutils.Pop3Client.forServer(server);
			if (encrypted) {
			...
			net.markwalder.vtestmail.testutils.Pop3Client client = clientBuilder.build();
			...
			Assertions.assertThat(commands).hasSize(authType.equals("USER") ? 5 : 4);
