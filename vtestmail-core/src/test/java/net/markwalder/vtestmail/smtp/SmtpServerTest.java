...
import net.markwalder.vtestmail.testutils.SmtpClient;
...
class SmtpServerTest {
	...
	void testData() throws IOException, MessagingException, InterruptedException {
		...
		try (SmtpServer server = new SmtpServer(store)) {
			...
			// prepare: SMTP client
			net.markwalder.vtestmail.testutils.SmtpClient client = net.markwalder.vtestmail.testutils.SmtpClient.forServer(server).withEHLO(false).build();
	...
	private void testEncryption(String sslProtocol, boolean useStartTLS) throws IOException, MessagingException, InterruptedException {
		...
		try (SmtpServer server = new SmtpServer(store)) {
			...
			// prepare: SMTP client
			net.markwalder.vtestmail.testutils.SmtpClient client;
			if (useStartTLS) {
				client = net.markwalder.vtestmail.testutils.SmtpClient.forServer(server)
	...
	private void testAuthentication(String authType, boolean encrypted, boolean wrongPassword) throws IOException, MessagingException, InterruptedException {
		...
		try (SmtpServer server = new SmtpServer(store)) {
			...
			// prepare: SMTP client
			net.markwalder.vtestmail.testutils.SmtpClient.SmtpClientBuilder clientBuilder = net.markwalder.vtestmail.testutils.SmtpClient.forServer(server);
			if (encrypted) {
			...
			net.markwalder.vtestmail.testutils.SmtpClient client = clientBuilder.build();
